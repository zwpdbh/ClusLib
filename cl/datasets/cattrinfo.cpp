...
namespace ClusLib {
    ...
    /**create a copy of the underlying object by using the default copy constructor of class*/
    ...
    Real CAttrInfo::distance(const AttrValue &av1, const AttrValue &av2) const {
        ...
        /**operator ^ is the exclusive or operator, it is true if and only only one operand is true*/
        if (is_unknown(av1) ^ is_unknown(av2)) {
            return 1.0;
        }
