...
namespace ClusLib {
    ...
    /**This is called forward declaration to avoid including the headers of classes DAttrInfo and CAttrInfo,
     * since such inclusion will result in cyclic inclusion of headers*/
    class DAttrInfo;
    class CAttrInfo;
    ...
    public:
        ...
        /**is used to access and modify the name of the attribute*/
        std::string& name();
        /**is used to get the type of the attribute. Once an attribute is created, we cannot change its type.
         * Hence, this function is declared as const function*/
        AttrType type() const;
        
        /**Those two overloaded operators are used to compare two objects of class AttrInfo
         * If two attributes have the same name and the same type, they are deemed equal.*/
        virtual bool operator==(const AttrInfo& info) const;
        virtual bool operator!=(const AttrInfo& info) const;
        
        /**classes derived from this class must override this function by implementing a deep copy*/
        virtual AttrInfo* clone() const = 0;
        
        /**It provides a uniform approach to calculate the distances for different types of data*/
        virtual Real distance(const AttrValue&, const AttrValue&) const = 0;
        
        /**get and set discrete value or continuous value, the derived classes are supposed to
        ...
        /**are used to deal with missing values as missing values are very common in data sets.
         * Missing values of different types are represented by different values, hence we leave
         * the definition of these function to derived classes*/
    ...
    private:
        ...
        std::string _name;
        /**The type of the attribute which is an enum, could be unknown, continuous or discrete*/
        AttrType _type;
    };
