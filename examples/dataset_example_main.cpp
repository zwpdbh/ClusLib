...
int main(int argc, char* argv[]) {
    try {
        /*an object of Schema is responsible to set the label and the identifier of an object of Record.*/ 
        ...
        schema->labelInfo() = labelInfo;    // labelInfo is a shared pointer pointing to an object of DAttrInfo can contains the categories of the input dataset.
...
//            cout << "schema is about to add: " << ai->name() << endl;
            schema->add(ai);
        }
        ...
        for (Size i = 0; i < ds->size(); i++) {
            ...
            for (Size j = 0; j < ds->num_attr(); j++) {
                if ((*schema)[j]->can_cast_to_c()) {
                    cout << setw(10) << left << (*schema)[j]->get_c_val((*ds)(i, j));
                } else {
                    cout << setw(10) << left << (*schema)[j]->get_d_val((*ds)(i, j));
                }
            }
